...
mat_data_type relu(mat_data_type x) { return x > 0 ? x : 0; }
...
nn_cfg_t nn_cfg_init(mat_data_type rate, mat_data_type lambda, ui64 batch_size,
                     ui64 epochs) {
  return ((nn_cfg_t){.rate = rate,
                     .lambda = lambda,
...
nn_t nn_init(mem_arena_t *arena, ui64 *arch, ui64 arch_count, char *errbuf) {
  ...
  assert(arena != NULL);
  nn_t nn = INIT_NN;
  size_t ok = nn_alloc(arena, &nn, arch, arch_count, errbuf);
  if (ok == -1) {
    return INIT_NN;
  }
  for (ui64 i = 0; i < arch_count - 1; i++) {
...
void nn_learn(nn_t *nn, nn_t *grads, mat_data_type rate, mat_data_type lambda,
              ui64 n) {
  assert(nn != NULL && grads != NULL);
  mat_data_type decay = 1.0f - (rate * lambda / n);
  if (decay < 0.0f)
    decay = 0.0f;
  for (ui64 i = 0; i < nn->count; i++) {
    mat_mult_scal(&nn->ws[i], &nn->ws[i], decay);
...
void nn_train(mem_arena_t *arena, nn_t *nn, matrix_t *input, matrix_t *target,
  ...
  for (ui64 e = 0; e < cfg->epochs; e++) {
    ...
    for (ui64 s = 0; s < simples; s += cfg->batch_size) {
      ...
      nn_learn(nn, &batch_grads, cfg->rate, cfg->lambda, simples);
